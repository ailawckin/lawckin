-- Include expertise areas (with years) for card display
  ...
  LEFT JOIN LATERAL (
    SELECT COALESCE(
      jsonb_agg(
        ...
        ORDER BY le.years_experience DESC NULLS LAST, pa.name
      ),
      '[]'::jsonb
    ) as expertise_areas
    FROM public.lawyer_expertise le
    JOIN public.practice_areas pa ON pa.id = le.practice_area_id
    WHERE le.lawyer_id = lp.id
  ) expertise ON true
