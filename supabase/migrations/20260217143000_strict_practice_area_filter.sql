-- Fix ambiguous column reference in search_lawyers_advanced (id vs output param)
...
BEGIN
  ...
  WITH weights AS (
    SELECT
      COALESCE((SELECT s.practice_area_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as practice_area_multiplier,
      COALESCE((SELECT s.location_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as location_multiplier,
      COALESCE((SELECT s.budget_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as budget_multiplier,
      COALESCE((SELECT s.specific_issue_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as specific_issue_multiplier,
      COALESCE((SELECT s.language_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as language_multiplier,
      COALESCE((SELECT s.keyword_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as keyword_multiplier,
      COALESCE((SELECT s.urgency_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as urgency_multiplier,
      COALESCE((SELECT s.rating_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as rating_multiplier,
      COALESCE((SELECT s.reviews_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as reviews_multiplier,
      COALESCE((SELECT s.views_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as views_multiplier,
      COALESCE((SELECT s.semantic_multiplier FROM public.search_weight_settings s WHERE s.id = 1), 1) as semantic_multiplier,
      COALESCE((SELECT s.new_lawyer_bonus FROM public.search_weight_settings s WHERE s.id = 1), 4) as new_lawyer_bonus
  ),
  ...
  lawyer_data AS (
    ...
    WHERE COALESCE(lp.verified, false) = true
      ...
      AND (p_embedding IS NULL OR lp.id IN (SELECT sc.id FROM semantic_candidates sc))
      ...
      (
        CASE
            ...
            WHEN ld.practice_areas IS NOT NULL AND EXISTS (
              ...
              WHERE LOWER(area) = LOWER(p_practice_area)
            ) THEN 40
            WHEN ld.specialty IS NOT NULL AND LOWER(ld.specialty) = LOWER(p_practice_area) THEN 38
            WHEN EXISTS (
              SELECT 1 FROM jsonb_array_elements(ld.all_expertise) elem
              WHERE LOWER(elem->>'area') = LOWER(p_practice_area)
            ) THEN 34
            WHEN EXISTS (
              SELECT 1 FROM jsonb_array_elements(ld.all_specializations) elem
              WHERE LOWER(elem->>'name') = LOWER(p_practice_area)
            ) THEN 30
            ELSE 0
  ...
  WHERE sl.score >= 20
    ...
    AND (
      p_practice_area IS NULL OR
      (
        sl.practice_areas IS NOT NULL AND EXISTS (
          SELECT 1 FROM unnest(sl.practice_areas) area
          WHERE LOWER(area) = LOWER(p_practice_area)
        )
      ) OR
      (sl.specialty IS NOT NULL AND LOWER(sl.specialty) = LOWER(p_practice_area)) OR
      EXISTS (
        SELECT 1 FROM jsonb_array_elements(sl.all_expertise) elem
        WHERE LOWER(elem->>'area') = LOWER(p_practice_area)
      )
    )
  ORDER BY
    ...
    SELECT
      ...
      (
        CASE
          ...
          WHEN ld.specialty IS NOT NULL AND LOWER(ld.specialty) = LOWER(p_practice_area) THEN 40
          WHEN EXISTS (
            SELECT 1 FROM jsonb_array_elements(ld.all_expertise) elem
            WHERE LOWER(elem->>'area') = LOWER(p_practice_area)
          ) THEN 35
...
-- After applying this migration, reload PostgREST schema so the API picks up the changes:
